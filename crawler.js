...
async function main() {
  try {
    ...
    const HG_RTN_TOP_200 = {
      packName: "HG, RTN 포함 Top Price 200 스페셜팩 (9강, 100+)",
      playerPrice: [],
    };
    const UT_TOP_800 = {
    ...
    const HG_RTN_LIST = await playerSearch([283, 284, 281, 274, 268, 270], 100); // playerSearch(시즌넘버, 최소오버롤)
    let HG_RTN_RESULTS = await playerPriceValue(HG_RTN_LIST, 9); // playerPriceValue(데이터 , 강화등급)
    ...
    // -------------------------------------- UT,24KB, JNM, 24HEROES, DC, JVA, CC, FCA, 23HW, HG, RTN, 23HEROES, RMCF, LN, SPL, 23NG, LOL, FA, 23KFA, 22HEROES, BTB, CAP, CFA, EBS, BOE21, NTG, UP, 22KFA, 2012KH_TOP_800--------------------------------------

    const UT_LIST = await playerSearch(
      [
        814, 830, 813, 811, 802, 801, 289, 290, 291, 283, 284, 281, 268, 270,
        274, 804, 265, 264, 806, 261, 256, 252, 254, 251, 253, 249, 246, 293,
        247,
      ],
      103
    ...
    const UT_FINAL = SortAndSlice(UT_RESULTS, 800); // SortAndSlice(데이터 , 자르기숫자)
    ...
    for (let item of UT_FINAL) {
      ...
      if (playerDocs.length > 0 && playerDocs[0]._id) {
        ...
        UT_TOP_800.playerPrice.push(playerData);
      }
    }
    data.seasonPack.push({ ...UT_TOP_800 });
    // -------------------------------------- LN_TOP_85--------------------------------------
